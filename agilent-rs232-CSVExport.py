...
#defaults
port = "COM6" #check port is right with windows device manager
baud = 57600
channel = 1
length = 1000
...
numSamples = len(data_points)
samplePeriod = scope_x_increment * 1000000 #in microseconds
captureLength = numSamples * scope_x_increment #in seconds
...
#plt.text(-0.02, -0.3, f'Sample Period: {samplePeriod} us/sample', style='normal',
        #bbox={'facecolor': 'grey', 'alpha': 0.5, 'pad': 10})
plt.text(0.2, -0.4, f'Sample Period: {samplePeriod:.2f} us/sample', 
        style='normal',
        transform=plt.gca().transAxes,
        bbox={'facecolor': 'grey', 'alpha': 0.5, 'pad': 10})
plt.text(0.2, -0.6, f'Capture length: {captureLength:.2f} s', 
        style='normal',
        transform=plt.gca().transAxes,
        bbox={'facecolor': 'grey', 'alpha': 0.5, 'pad': 10})
plt.subplots_adjust(bottom=0.4)  # leave more space at the bottom
